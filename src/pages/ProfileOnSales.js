...
import { NFT_ABI,COLLECTION_ABI,MARKET_ABI } from "../NFT_ABI";
import Web3 from 'web3';
import NfToken from "../components/NfToken";
...
const Profile = () => {
  let navigate = useNavigate()
    ...
    let [key,setKey] = useState('onsale')
...
useEffect(()=>{
  files.length>0 && updateCoverImg(user.user._id,files[0])
},[files])

const updateCoverImg=async (regUser,coverImage)=>{
  ...
  try {
    ...
    let res=await axios.patch(`${API_URL}/coverimg/${regUser}`,formData,{withCredentials:true}, headers)
    console.log(res,"res")
...
let GetData = (()=>{
  if(user && user.user && user.user.address){
    let  collectionContractAddress = '0x2E73fa6d643EeB770aA26be13eF0dA7C697d3561';
    let userAddress = user&& user.user.address; //Metamask logged in Address
   const contractInstance = new web3.eth.Contract(COLLECTION_ABI, collectionContractAddress);
    contractInstance.methods.userCollections(userAddress).call().then((data)=>{
...
useEffect(()=>{
  ...
  collectionList && collectionList.length && collectionList.map(async(item)=>{
    let nftContractAddress = item.newCollection; 
   let marketContractAddress = "0xD6C5092e4E932d17FB41a22BB57693fE4109fFaD"
   let  userAddress = user &&user.user &&user.user.address;
    // Contract instance
    ...
    const contractInstanceNFT = new web3.eth.Contract(NFT_ABI, nftContractAddress);
    // Find gas limit
    let result = await contractInstance.methods
      .getUserTokensOnSale(userAddress, nftContractAddress)
      .call();
    for (var i = 0; i < result.length; i++) {
      let uri = await contractInstanceNFT.methods.getTokenIPFSPath(result[i]).call();
      array.push({ipfs:uri,token:result[i],collectionId:item.newCollection,collectionHash:item.ipfsHash})
      console.log(result)
    }
    console.log(result, array);
    let uniq = [...new Set(array)];
    setipfshashArray(uniq)
  })
},[collectionList,])
...
const fetchUserFollowing = async () => {
  let { data }=await axios.get(`${API_URL}/get-follow-data/${user.user._id}`)
  setFollowData(data.followData)
  setFavouriteData(data.favourites)
}


const handleFollow = async (loggedInUserId, userId) => {
  try {
    let { data }=await axios.post(`${API_URL}/follow-user/${userId}/${loggedInUserId}`)
    const isFollow = data && data.map((usr) => {
      usr.user === userId
    }).length > 0
...
useEffect(() => {
  if(key === 'created'){
     ...
     window.location.href = "/profile/collectibles"
   }
 },[key])
...
return (
    <>
    {isloading ?  
    <div style={{height:"80vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--background)"}}>
 <ReactLoading type={'bubbles'} color={'var(--background2)'} height={'10%'} width={'10%'} />
</div>   : <div>
 <div>
   <div
     style={{
       backgroundImage: user.user.coverimg ? `url(${user.user.coverimg})`:"url(/images/profileBanner.png)",
       height: "320px",
   ...
   >
     <div style={{ display: "flex", justifyContent: "space-between" }}>
       ...
       >
          <img
           style={{ marginLeft: "20%", marginBottom: "5%",borderRadius:"50%",height:"150px",width:"150px"  }}
           src={`${user.user&& user.user.imgpath}`}
         />{" "}
         <br></br>
         <h4
           style={{ color: "white", textAlign: "center", marginTop: "5%",wordWrap:"break-word",width:"100%" }}
         >
           { user && user.user  ? user.user.name : ""} <br></br>{" "}
           <span style={{ fontSize: "0.9rem",textAlign:"center"}}>
             {
               user && user.user&&user.user.address  ?  formatAddress(user.user.address) : ""
             }
             <svg
               width="16"
               height="16"
  ...
  <Tabs
   ...
   <div
     style={{
       ...
       gap: "8%",
       flexWrap:"wrap",
       paddingBottom:"5%",marginLeft:"5%",marginTop:"5%",
       height: "fit-content",paddingBottom:"80vh"
     }}
     
   >
   {!ipfshashArray.length   ?
   <div style={{flexBasis:"100%" }}>
    <div style={{display:"flex",justifyContent:"center"}}>
    
      </div>
</div>
    :ipfshashArray &&ipfshashArray.length && ipfshashArray.map((item)=>(
      <NfToken item={item}  />
    ))
   }
      ...
      <Tab eventKey="collectibles" title="Collectibles">
        <div style={{height: "fit-content",paddingBottom:"80vh"}}>
      ...
      <Tab eventKey="likes" title="Likes" style={{height: '70vh', color: 'whitesmoke'}}>
   <div
     style={{
       display: "flex",
       gap: "8%",
   ...
   <div style={{flexBasis:"100%" }}>
    ...
    :favouriteData && favouriteData.length > 0 && favouriteData.map((item)=>(
      <NfToken item={item}  />
    ))
   }
   </div>
      </Tab>
      <Tab eventKey="following" title="Following" style={{color:"whitesmoke", height: '70vh'}}>
      {followData && followData?.following?.length === 0 ? (
          <p>No followings found.</p>
        ) : (followData && followData?.following?.map((flw) => (
          <div
          style={{
            paddingBottom: "10%",
            display: "grid",
        ...
        >
              <div style={{display:"grid",gridTemplateColumns:"30% 50%",marginTop:"5%",gap:"15%",height:"5%"}}>
                <img height={103} width={100}  src={flw?.user?.imgpath} style={{borderRadius: "50%"}}/>
                <div>                <p style={{color:"white",marginLeft:"10%",
                  }}>{flw?.user?.name}<br></br><span style={{color:"var(--background2)"}}>{flw?.followersCount} followers</span></p>
                        <p style={{color:"var(--background2)"}}></p>
                          <Button
                              ...
                              onClick={() => handleFollow(user.user._id,flw?.user?._id)}
                          >
                              {flw.isFollowing || isFollow ? 'Unfollow' : 'Follow'}
                          </Button>
                </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"23% 23% 23% 23%", marginTop: "1rem"}}>
            {flw?.userNfts.length > 0 && flw.userNfts.map((userNft) => (
              <div style={{margin: "0 10px", height: "100px", width: "100px", cursor: "pointer"}} onClick={()=>{
                window.location.href = `/item-1/${userNft?.ipfs}/${userNft?.token}/${userNft?.collectionId}`
            }}>
                <img  src={userNft.imgpath} style={{objectFit: "cover !important",height: "100%", width: "100%"}} />
                </div>
                ))}
                </div>
        </div>
      ...
      <Tab eventKey="followers" title="Followers" style={{color: "whitesmoke", height: '70vh'}}>
        {followData && followData?.followers?.length === 0 ? (
          <p style={{paddingBottom:"50vh"}}>No followers found.</p>
        ) : (followData && followData?.followers?.map((flw) => (
          <div
          style={{
            paddingBottom: "10%",
            display: "grid",
        ...
        >
              <div style={{display:"grid",gridTemplateColumns:"30% 50%",marginTop:"5%",gap:"15%",height:"5%"}}>
                <img height={103}  src={flw?.user?.imgpath} style={{borderRadius: "50%"}} />
                <div>                <p style={{color:"white",marginLeft:"10%",
                  }}>{flw?.user?.name}<br></br><span style={{color:"var(--background2)"}}>{flw?.followersCount} followers</span></p>
                        <p style={{color:"var(--background2)"}}></p>
                          <Button
                              ...
                              onClick={() => handleFollow(user.user._id,flw?.user?._id)}
                          >
                            {flw?.isFollowing ? 'Unfollow' : 'Follow'}
                          </Button>{" "}   
                </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"23% 23% 23% 23%", marginTop: "1rem"}}>
