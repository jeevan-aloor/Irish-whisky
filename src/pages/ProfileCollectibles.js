...
let array = []
  ...
  useEffect(()=>{
      ...
      collectionList && collectionList.length && collectionList.map(async(item)=>{
        console.log(item)
        let nftContractAddress = item.Collections;
      ...
      for (var i = 0; i < bal; i++) {
        ...
        array.push({ipfs:uri,token:id,collectionId:item.Collections})
 ...
 >
   ...
   >
     ...
     >
       ...
       <p          onClick={()=>window.location.href ="/profile/collectibles"}
 style={{ margin: "1rem",cursor:"pointer" , background: "var(--background2)",
           padding: "0.1rem",
           color: "black", }}>COLLECTIBLES</p>
